...
export class Network extends pulumi.ComponentResource {
    ...
    constructor(name: string, NetworkOptions: NetworkOptions, opts?: pulumi.ResourceOptions) {
        ...
        this.publicSubnets = this.createSubnets(this.NetworkOptions.publicSubnetsOptions, true);

        // Private network
        this.createNatGateways(this.publicSubnets[0]);
        this.privateSubnets = this.createSubnets(this.NetworkOptions.privateSubnetsOptions, false);
    }

    private createVpc() {
        this.mainVpc = new aws.ec2.Vpc(this.NetworkOptions.vpcOptions.name, {
            cidrBlock: this.NetworkOptions.vpcOptions.cidrBlock,
            instanceTenancy: "default",
            enableDnsHostnames: true,

            tags: {
                Name: `${this.NetworkOptions.vpcOptions.name}-Vpc`
            }
        }, { parent: this });
    }
    ...
    private createInternetGateway() {
        this.internetGateway = new aws.ec2.InternetGateway("internetGateway", {
            ...
            tags: {
                Name: `${this.NetworkOptions.vpcOptions.name}-Internet-Gateway`,
            },
        }, { parent: this.mainVpc });
        ...
        this.publicRouteTable = new aws.ec2.RouteTable("publicRouteTable", {
            ...
            tags: Object.assign({}, this.defaultTags, {
                Name: `${this.NetworkOptions.vpcOptions.name}-Public-RouteTable`,
            }),
    ...
    private createNatGateways(publicSubnet: aws.ec2.Subnet) {
        ...
        const elasticIp = new aws.ec2.Eip("elasticIp", {
            tags: {
                Name: `${this.NetworkOptions.vpcOptions.name}-ElasticIp`
            }
        }, { parent: publicSubnet });
        ...
        this.awsNatGateway = new aws.ec2.NatGateway("natGatewayPrivateSubnet", {
            ...
            tags: Object.assign({}, this.defaultTags, {
                Name: `${this.NetworkOptions.vpcOptions.name}-Nat-Gateway`,
            }),
        }, { parent: publicSubnet });
        ...
        this.privateRouteTable = new aws.ec2.RouteTable("privateRoutetable", {
            vpcId: this.mainVpc.id,
            tags: Object.assign({}, this.defaultTags, {
                Name: `${this.NetworkOptions.vpcOptions.name}-Private-RouteTable`,
            }),
